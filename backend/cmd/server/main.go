...
func main() {
	...
	if e = db.AutoMigrate(&model.User{}, &model.Mood{}, &model.Assessment{}, &model.Journal{}, &model.UserAssessment{}, &model.MoodCheckIn{}); e != nil {
	...
	cr := repository.NewMoodCheckInRepository(db)
	ar := repository.NewAssessmentRepository(db)
	jr := repository.NewJournalRepository(db)
	us := service.NewUserService(ur, logger)
	cs := service.NewMoodCheckInService(cr, mr, logger)
	ms := service.NewMoodService(mr, cs, logger)
	...
	h := router.Handlers{User: handler.NewUserHandler(us, as, logger, cfg.JWTSecret, cfg.JWTIssuer), Mood: handler.NewMoodHandler(ms, logger), CheckIn: handler.NewMoodCheckInHandler(cs, logger), Assessment: handler.NewAssessmentHandler(as, logger), Journal: handler.NewJournalHandler(js, logger)}
